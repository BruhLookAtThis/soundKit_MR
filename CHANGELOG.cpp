...
- Tested on a know problem batch of like 25 sounds, checked out on all of them
...
For soundMod
- Bank_Stream .wem files are now all converted to decimal instead of remaining hexadecimals. This change is in preparation for planned methods of finding/mapping SFX .wems. If you log SFX on the Spreadsheet, log BOTH the decimal and the hexadecimal number for the wem ID. All the hexadecimal numbers should be 8-digits long, btw. You can go to this website if you need a number converted:
...
- Place a decimal/hexadecimal number to the front of the files name, separated from the rest of the by a hyphen or underscore if need be, then place the file back into "2-Put-Custom-Wems-In-Here" and run the script again (if youre still gonna use that wem at all)

- Added ability to detect .bnk-less .wem files
- These files will now be moved to a "6-Wems-With-No-Bnk-Match-Here" folder
- Place a .bnk file that uses that .wem ID into 1-Put-Vanilla-Bnks-In-Here, then place the file back into "2-Put-Custom-Wems-In-Here" and run the script again (if youre still gonna use that wem at all)
