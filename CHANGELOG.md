...
- Fixed Option 1 Sub-Option 3 & 5 adding files that did not have a decimal change to the Update Log file.
...
- Updated AUDIO-ID-LIST.txt file
- Adjusted DEVELOPER Option 200 to output into `0_XTRA/0-DEV/DEV-200-AMPED-AUDIO` folder
...
- Added DEV Option 202. This option copies bnks from `3-Your-MODDED-BNK-Is-Here` into
  `0_XTRA\0-DEV\DEV-202-REPAKED-BNKS\1-PUT-BNK-FILES-HERE`, then automatically runs the z2_DoubleClickMe_RPK_AUTO_MR.bat
...
For SMT
-Added logs. Every executed Option now has its terminal logs output to a .txt file, placed in 0_XTRA/0-LOGS-SMT
...
- Renamed `Misc-Tools` to `3-RANDOM-Tools`
- Renamed `File-In-Folder-Lister_v1.1.0` to `2-FILE-IN-FOLDER-Lister_v1.1.0`
- Renamed `Folder-Path-Maker_v1` to `3-FOLDER-PATH-Maker_v1`
- Added `1-WORK-BNK-Updater` to `1-MISC-STUFF`
  - To use this tool, extract the `WwiseAudio` folder from FModel as `uassets` (this will give you latest .bnk files)
  - Place the English .bnk files in `0_XTRA\1-MISC-STUFF\3-RANDOM-Tools\1-WORK-BNK-Updater\0-PUT-ALL-NEW-BNKS-HERE\English(US)`
    - You can delete the Chinese and Japanese .bnks
  - Place the remaining .bnk files in `0_XTRA\1-MISC-STUFF\3-RANDOM-Tools\1-WORK-BNK-Updater\0-PUT-ALL-NEW-BNKS-HERE\`
  - Place all vanilla .bnks that you use for your mod making in `1-PUT-ALL-WORK-BNKS-HERE`
...
- Added Sub-Option 7 to Option 1
  ...
  - Users will place wems in numbered folders inside  of `2-Put-CUSTOM-WEMs-Here`
    - For Example: `2-Put-CUSTOM-WEMs-Here\1\WEMFILES.wem`, `2-Put-CUSTOM-WEMs-Here\2\WEMFILES.wem`,
  - Then they will create 0-PAK-NAME-LIST-#.txt file(s) (if not already generated by the script) in `0_XTRA\0-TXTs\`
  ...
  - So if I have a `2-Put-CUSTOM-WEMs-Here\1\` folder, then I will also have a 0-PAK-NAME-LIST-1.txt file
    - You can add another hyphen after the number to add notes to your file
    - For Example: `0-PAK-NAME-LIST-1-ForLowVolumeMods.txt`
  ...
  - There is more details inside of the `0-PAK-NAME-LIST-#.txt` provided in the download, so just read that
  - Continuing, when selecting this sub-option in the script, users will be taken to a new menu
...
For SDKT
- Removed the `Display` folder output from Option S
  - Now ALL sorted SFX, including all `display` files, will be in the `SFX` folder

NOTE: This is NOT heavily tested! 

----------------------------------------------------------------------------------------------------------
----------------------------------------------------------------------------------------------------------
UPDATE 2.2.4b (07-02-2026)
...
----------------------------------------------------------------------------------------------------------
----------------------------------------------------------------------------------------------------------
UPDATE 2.2.4c (07-9-2026)

For General

For CVS
- Updated Character ID List (7-09-2026)
...
- Updated Option 1 Sub-Option 2 and 3 + Option 102 Sub-Options 1 and 2 to create .wav file versions of the extracted bnk folders
  - These folders will have the same names as the originals, just with `-WAVs` appended
  - The Option 102 Sub-Options `-WAV` folders will not move if you select `y` on any transfer prompts
  - Thats because, obviously, .wav files cant be injected into .bnks/.paks
...
- Updated Option 1 Sub-Option 4 & 5 to choose default costume filenames if an alt costume shares a newer wem ID with it
  - Not really important, made this for myself really
...
- Fixed Option 1 Sub-Option 5-8 making separate .pak files for `0-PAK-NAME-LIST.txt` entries that shared an assigned output .pak name
...
----------------------------------------------------------------------------------------------------------
----------------------------------------------------------------------------------------------------------
UPDATE 2.9.9-beta-19 (07-30-2026)
### (THIS VERSION IS MEANT FOR BUG TESTING. REPORT ANY ISSUES TO @BruhLookAtThis on Discord/ @BruhiookAtThis on Twitter)
...
----------------------------------------------------------------------------------------------------------
----------------------------------------------------------------------------------------------------------
UPDATE 2.9.9-beta-21b (08-09-2026)
### (THIS VERSION IS MEANT FOR BUG TESTING. REPORT ANY ISSUES TO @BruhLookAtThis on Discord/ @BruhiookAtThis on Twitter)

For SDKT
- Fixed Option 0 not working properly
...
----------------------------------------------------------------------------------------------------------
----------------------------------------------------------------------------------------------------------
## UPDATE 2.9.9-beta-24c (09-19-2026) (NOT RELEASED YET, `BUG TESTING`)
...
- Merged Option 1 Sub-Option 9 into Sub-Options 6/7 (teehee)
- Adjusted Option 1 Sub-Options 6/7 to skip attempting to process `.wem`s within `Media` subfolders into `.bnk`s
- Renamed Option 1 Sub-Option 10 to Sub-Option 9
